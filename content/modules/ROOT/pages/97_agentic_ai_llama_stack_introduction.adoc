...
Most of the llama stack server configuration is done via a yaml file called `run.yaml`. link:https://llamastack.github.io/docs/distributions/configuration[Click here for detailed documentation]. When using the operator, this configuration is stored within a config map.
